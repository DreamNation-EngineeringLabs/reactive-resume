...
export const resumeComment = pg.pgTable(
	"resume_comment",
	{
		id: pg
			...
			.primaryKey()
			.$defaultFn(() => generateId()),
		resumeId: pg.uuid("resume_id").notNull().references(() => resume.id, { onDelete: "cascade" }),
		tenantId: pg.text("tenant_id").notNull(), // Reference to eng-labs tenant
		authorId: pg.uuid("author_id").notNull(), // eng-labs user ID (faculty/PO)
		studentId: pg.uuid("student_id").notNull(), // eng-labs user ID (student)
		content: pg.text("content").notNull(),
		scope: pg.text("scope").notNull().default("INDIVIDUAL"), // INDIVIDUAL | SECTION
		status: pg.text("status").notNull().default("DRAFT"), // DRAFT | PUBLISHED | RESOLVED
...
export const resumeChecklist = pg.pgTable(
	"resume_checklist",
	{
		id: pg
			...
			.primaryKey()
			.$defaultFn(() => generateId()),
		facultyId: pg.uuid("faculty_id").notNull(), // eng-labs faculty user ID
		tenantId: pg.text("tenant_id").notNull(), // Reference to eng-labs tenant
...
export const resumeEvaluation = pg.pgTable(
	"resume_evaluation",
	{
		id: pg
			...
			.$defaultFn(() => generateId()),
		resumeId: pg.uuid("resume_id").notNull().references(() => resume.id, { onDelete: "cascade" }),
		studentId: pg.uuid("student_id").notNull(), // eng-labs user ID
		tenantId: pg.text("tenant_id").notNull(), // Reference to eng-labs tenant
		checklistId: pg
			.uuid("checklist_id")
			.notNull()
		...
		evaluatedBy: pg.uuid("evaluated_by").notNull(), // eng-labs faculty/PO user ID
...
export const resumeHistory = pg.pgTable(
	"resume_history",
	{
		...
		changedBy: pg.uuid("changed_by").notNull(), // eng-labs user ID
		actorType: pg.text("actor_type").notNull(), // STUDENT | FACULTY | PO | ADMIN
		createdAt: pg.timestamp("created_at", { withTimezone: true }).notNull().defaultNow(),
	},
	(t) => [
		pg.index().on(t.resumeId),
