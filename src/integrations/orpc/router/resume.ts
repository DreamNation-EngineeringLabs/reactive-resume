...
// not Better Auth's `user` table. Resolve the authenticated user's eng-labs identity
// by email before writing those rows.
async function resolveEngLabsActorId(email: string): Promise<string> {
	...
	comments: {
		create: protectedProcedure
			...
			.handler(async ({ context, input }) => {
				...
				const authorId = await resolveEngLabsActorId(context.user.email);
	...
	checklists: {
		create: protectedProcedure
			...
			.handler(async ({ context, input }) => {
				...
				const facultyId = await resolveEngLabsActorId(context.user.email);
	...
	evaluations: {
		create: protectedProcedure
			...
			.handler(async ({ context, input }) => {
				...
				const evaluatedBy = await resolveEngLabsActorId(context.user.email);
