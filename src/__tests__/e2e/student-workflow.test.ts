...
describe("Student Resume Workflow (E2E)", () => {
	...
	describe("Student Comment Receipt & Notifications", () => {
		...
		it("should mark notification as read", async () => {
			...
			if (notifResponse.body.responseData.notifications.length > 0) {
				const notifId = notifResponse.body.responseData.notifications[0].id;
	...
	describe("Student Dashboard Access", () => {
		it("should retrieve student dashboard", async () => {
			...
			const response = await request(app).get("/v1/student/dashboard").set("Authorization", `Bearer ${student.token}`);
	...
	describe("Permission & Tenant Isolation", () => {
		it("should enforce tenant scoping for student data", async () => {
			...
			const response = await request(app).get("/v1/resumes/user/me").set("Authorization", `Bearer ${student.token}`);
		...
		it("should reject expired tokens", async () => {
			...
			const response = await request(app).get("/v1/resumes/user/me").set("Authorization", `Bearer ${expiredToken}`);
