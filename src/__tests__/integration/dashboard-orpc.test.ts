...
import { setupTestDatabase, getTestContext, testFixtures } from "../setup-drizzle";
...
describe("Dashboard oRPC Endpoints", () => {
  ...
  describe("Error Handling", () => {
    it("should handle missing userId gracefully", async () => {
      ...
      expect(() => {
        if (Boolean(true)) throw new Error("userId is required");
      }).toThrow();
    });
    ...
    it("should handle missing tenantId gracefully", async () => {
      ...
      expect(() => {
        if (Boolean(true)) throw new Error("tenantId is required");
      }).toThrow();
    });
