...
describe("Resume API Endpoints", () => {
	describe("Student Resume Operations", () => {
		it("should retrieve student's own resumes with pagination", () => {
			...
			const mockResumes = [testFixtures.resumes.resume1, testFixtures.resumes.resume2];

			// Both should belong to the student
			expect(mockResumes.every((r) => r.studentId === context.userId)).toBe(true);
		});
	...
	describe("Checklist Management", () => {
		...
		it("should validate checklist items have weights", () => {
			...
			const allItemsHaveWeights = checklist.items.every((item) => item.weight !== undefined && item.weight > 0);
	...
	describe("Evaluation Management", () => {
		...
		it("should retrieve evaluations for a resume", () => {
			...
			expect(evaluations.every((e) => e.overallScore >= 0 && e.overallScore <= 5)).toBe(true);
		});
	...
	describe("Admin Dashboard Access", () => {
		...
		it("should allow date range filtering for statistics", () => {
			...
			expect(stats.resumesSubmitted).toBeGreaterThanOrEqual(stats.evaluationsCompleted);
		});
	});
